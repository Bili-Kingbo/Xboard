...
class SubscribeTemplate extends Model
{
    ...
    public static function getContent(string $name): ?string
    {
        ...
        return Cache::store(config('app.settings_cache_store', 'redis'))->remember($cacheKey, 3600, function () use ($name) {
    ...
    public static function setContent(string $name, ?string $content): void
    {
        ...
        Cache::store(config('app.settings_cache_store', 'redis'))->forget(self::$cachePrefix . $name);
    }
